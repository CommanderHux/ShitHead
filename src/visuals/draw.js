...
export function drawShapeOnCanvas(canvas, shape) {
...
export function getCharacterWidth(context, character) {
...
export class Card extends Rect {
    constructor(
    /** card ID */ id, x, y, w = 10, h = 10, f = "black", s = "black") {
        super(x, y, w, h);
        this.id = id;
        this.suit = id % 4;
        this.value = id % 13;
...
function getCharacterRatio(context, character) {
    const metrics = context.measureText(character);
    const width = metrics.width;
    const height = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
    return height === 0 ? 0 : width / height;
}
export function runDrawTest(canvas) {
    const context = canvas.getContext("2d");
    if (!context) {
        throw new Error("Could not get a 2D rendering context from the canvas.");
    }
