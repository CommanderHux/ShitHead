...
drawPile.onDown = () => {
  if (getPlayer() instanceof AI) return;
...
export function onMouseDown(mouse: MouseEvent) {
  ...
  for (const shape of shapes) {
    ...
    if (!inRange(x, y, bounds.x, bounds.y, bounds.w, bounds.h)) continue;
      shape.onDown();
    break;
  }
};
...
function getShapeBounds(shape: Rect): { x: number; y: number; w: number; h: number } {
  if (shape instanceof Card) {
    return {
      x: shape.x,
...
function finishTurn(beforePlayer: number) {
  if (currentPlayer === beforePlayer) nextPlayer();
}

function runAiTurns() {
  if (!(getPlayer() instanceof AI) || aiTurnScheduled) return;
  aiTurnScheduled = true;
  window.setTimeout(() => {
    aiTurnScheduled = false;
    if (!(getPlayer() instanceof AI)) return;
    const ai = getPlayer();
    if (!(ai instanceof AI)) return;
    ...
    if (currentPlayer === before) nextPlayer();
  }, 350);
}

export function nextPlayer() {
  currentPlayer = (currentPlayer + 1) % players.length;
  if (getPlayer() instanceof AI) runAiTurns();
}
