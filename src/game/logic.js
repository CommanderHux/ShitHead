...
export function getCurrentZone(player) {
    if (player.hand.length > 0)
        return "hand";
    if (player.up.length > 0)
        return "up";
    return "down";
}
...
function canPlace(value, lastValue) {
    if (lastValue < 0)
        return true;
    if (value === 0)
        return true; // 2
    if (value === 8)
        return true; // 10
    if (lastValue === 5)
        return value <= lastValue; // on 7 play under
    return value >= lastValue;
}
function shouldBurn(ids) {
    if (ids.length === 0)
        return false;
    if (getIdValue(ids.at(-1)) === 8)
        return true;
    if (ids.length < 4)
        return false;
...
function refillHandToThree(state, seatIndex) {
...
function pickupDiscard(state, seatIndex) {
    const player = state.players[seatIndex];
    if (!player)
        return;
...
function applyPlayedCards(state) {
    const seatIndex = state.turn;
    const player = state.players[seatIndex];
    if (!player || state.selected.length === 0)
        return state;
...
function applyDrawFromPile(state) {
    const seatIndex = state.turn;
...
function applyPickupDiscard(state) {
    pickupDiscard(state, state.turn);
    clearSelection(state);
    advanceTurn(state);
    return state;
}
function applySwapSelected(state) {
...
function scoreOption(ids, value, count, lastValue, discardPile, hasNonWildPlayable) {
...
function chooseStrongest(ids, discardPile) {
    if (ids.length === 0)
        return [];
    ...
    for (const value of playableValues) {
        ...
        const score = scoreOption(ids, value, option.length, lastValue, discardPile, hasNonWildPlayable);
        if (score > bestScore ||
...
export function reduceGameState(state, action) {
    ...
    if (action.type === "selectCard") {
        ...
        if (!cards.includes(action.cardID))
            return state;
        if (zone === "down") {
            if (next.selected.includes(action.cardID))
                next.selected = [];
            else
                next.selected = [action.cardID];
            return next;
        }
        if (next.selected.includes(action.cardID)) {
            next.selected = next.selected.filter((id) => id !== action.cardID);
        ...
        if (getIdValue(action.cardID) !== activeValue)
            return state;
        next.selected.push(action.cardID);
        return next;
    }
...
export function canLocalSeatAct(state, localSeat, mode) {
    if (state.status !== "playing")
        return false;
    const turnPlayer = state.players[state.turn];
    if (!turnPlayer || turnPlayer.isAI)
        return false;
    if (mode === "local")
        return true;
