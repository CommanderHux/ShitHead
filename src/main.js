...
import { ACTION_BUTTON_H, ACTION_BUTTON_PRIMARY_Y_FROM_BOTTOM, ACTION_BUTTON_SECONDARY_Y_FROM_BOTTOM, ACTION_BUTTON_W, ACTION_BUTTON_X, CARD_COUNT_LABEL_OFFSET_Y, CARD_H, CARD_SELECTED_OFFSET_Y, CARD_STACK_X_GAP, CARD_STACK_Y_GAP, CARD_W, COLOR_BUTTON_BG, COLOR_BUTTON_BG_PRESSED, COLOR_GAME_BACKGROUND, COLOR_OVERLAY, COLOR_SETUP_BACKGROUND, COLOR_TEXT_BLACK, COLOR_TEXT_PRIMARY, COLOR_TEXT_SECONDARY, COLOR_TEXT_WHITE, COLOR_TURN_HIGHLIGHT, DISCARD_PILE_LABEL_X, DISCARD_PILE_LABEL_Y, DISCARD_PILE_X, DISCARD_PILE_Y, DRAW_PILE_LABEL_X, DRAW_PILE_LABEL_Y, DRAW_PILE_X, DRAW_PILE_Y, FONT_BODY, FONT_TITLE, MENU_BUTTON_H, MENU_BUTTON_W, MENU_BUTTON_X, MENU_BUTTON_Y_FROM_BOTTOM, PLAYER_NAME_OFFSET_Y, PLAYER_ZONE_HAND_OFFSET_Y, PLAYER_ZONE_LABEL_OFFSET_Y, PLAYER_ZONE_UP_OFFSET_Y, SEAT_LAYOUT, SETUP_AI_FILL_H, SETUP_AI_FILL_W, SETUP_AI_FILL_X, SETUP_AI_FILL_Y, SETUP_CANCEL_ONLINE_H, SETUP_CANCEL_ONLINE_W, SETUP_CANCEL_ONLINE_X, SETUP_CANCEL_ONLINE_Y, SETUP_MODE_LOCAL_H, SETUP_MODE_LOCAL_W, SETUP_MODE_LOCAL_X, SETUP_MODE_LOCAL_Y, SETUP_MODE_ONLINE_H, SETUP_MODE_ONLINE_W, SETUP_MODE_ONLINE_X, SETUP_MODE_ONLINE_Y, SETUP_NAME_H, SETUP_NAME_W, SETUP_NAME_X, SETUP_NAME_Y, SETUP_PLAYERS_H, SETUP_PLAYERS_W, SETUP_PLAYERS_X, SETUP_PLAYERS_Y, SETUP_ROLE_HOST_H, SETUP_ROLE_HOST_W, SETUP_ROLE_HOST_X, SETUP_ROLE_HOST_Y, SETUP_ROLE_JOIN_H, SETUP_ROLE_JOIN_W, SETUP_ROLE_JOIN_X, SETUP_ROLE_JOIN_Y, SETUP_ROOM_H, SETUP_ROOM_W, SETUP_ROOM_X, SETUP_ROOM_Y, SETUP_START_H, SETUP_START_W, SETUP_START_X, SETUP_START_Y, SETUP_STATUS_X, SETUP_STATUS_Y_FROM_BOTTOM, SETUP_TITLE_Y, STATUS_LINE_X, STATUS_LINE_Y, TURN_HIGHLIGHT_H, TURN_HIGHLIGHT_W, TURN_HIGHLIGHT_X_OFFSET, TURN_HIGHLIGHT_Y_OFFSET, } from "./visuals/cardSize.js";
...
export let drawPile = { cardIDs: [] };
export let discardPile = { cardIDs: [] };
...
function buildHostSeats(remotes) {
    const seats = [{ name: setupState.name || "Host", isAI: false, isLocal: true }];
...
function applyHostAction(seat, action) {
    if (!gameState || gameState.status !== "playing")
        return;
...
function scheduleAiTurn() {
    ...
    if (!current?.isAI)
        return;
    if (setupState.mode === "online" && !onlineSession?.isHost)
        return;
...
function getSeatDisplayIndex(realSeat) {
    if (!gameState)
        return realSeat;
    const total = gameState.players.length;
    if (setupState.mode === "local")
        return realSeat;
    return (realSeat - localSeat + total) % total;
}
function getSeatAnchor(displaySeat, totalPlayers) {
    ...
    if (totalPlayers === 3) {
        ...
        if (displaySeat === 1)
            return { x: SEAT_LAYOUT.three.leftX, y: SEAT_LAYOUT.three.leftY };
    ...
    if (displaySeat === 1)
        return { x: width / 2 + SEAT_LAYOUT.four.topXCenterOffset, y: SEAT_LAYOUT.four.topY };
    if (displaySeat === 2)
        return { x: SEAT_LAYOUT.four.leftX, y: height / 2 + SEAT_LAYOUT.four.leftYFromCenter };
    return {
        x: width - SEAT_LAYOUT.four.rightXFromRight,
        y: height / 2 + SEAT_LAYOUT.four.rightYFromCenter,
    };
}
function drawCardImage(cardID, x, y) {
...
function drawDeckZone(seatIndex, zone, cards, x, y) {
    ...
    const canSelect = canActLocally() && isTurnSeat && (isSwapPhase ? zone === "hand" || zone === "up" : currentTurnZone === zone);
    for (let i = 0; i < maxVisible; i++) {
        ...
        const shouldHide = zone === "down" ||
            (zone === "hand" && (seat?.isAI === true ||
...
function addButton(x, y, w, h, key, label, onDown) {
...
function drawGameScene() {
    ...
    state.players.forEach((player, seatIndex) => {
        ...
        if (isTurn && state.status === "playing") {
            ...
            context.strokeRect(anchor.x + TURN_HIGHLIGHT_X_OFFSET, anchor.y + TURN_HIGHLIGHT_Y_OFFSET, TURN_HIGHLIGHT_W, TURN_HIGHLIGHT_H);
        }
    ...
    if (state.phase === "swap") {
        addButton(ACTION_BUTTON_X, h - ACTION_BUTTON_PRIMARY_Y_FROM_BOTTOM, ACTION_BUTTON_W, ACTION_BUTTON_H, "swap", "Swap", () => dispatchAction({ type: "swapSelected" }));
        addButton(ACTION_BUTTON_X, h - ACTION_BUTTON_SECONDARY_Y_FROM_BOTTOM, ACTION_BUTTON_W, ACTION_BUTTON_H, "ready", "Ready", () => dispatchAction({ type: "readySwap" }));
    }
    else {
        addButton(ACTION_BUTTON_X, h - ACTION_BUTTON_PRIMARY_Y_FROM_BOTTOM, ACTION_BUTTON_W, ACTION_BUTTON_H, "play", "Play", () => dispatchAction({ type: "playSelected" }));
        addButton(ACTION_BUTTON_X, h - ACTION_BUTTON_SECONDARY_Y_FROM_BOTTOM, ACTION_BUTTON_W, ACTION_BUTTON_H, "sort", "Sort", () => dispatchAction({ type: "sortHand" }));
    }
...
function drawSetupScene() {
    ...
    addButton(SETUP_MODE_ONLINE_X, SETUP_MODE_ONLINE_Y, SETUP_MODE_ONLINE_W, SETUP_MODE_ONLINE_H, "mode-online", `Mode: Online`, () => {
    ...
    addButton(SETUP_ROOM_X, SETUP_ROOM_Y, SETUP_ROOM_W, SETUP_ROOM_H, "room", `Room: ${setupState.roomCode}`, () => {
        ...
        if (next != null && next.trim().length > 0)
            setupState.roomCode = next.trim().toUpperCase();
    });
    addButton(SETUP_PLAYERS_X, SETUP_PLAYERS_Y, SETUP_PLAYERS_W, SETUP_PLAYERS_H, "players", `Players: ${setupState.playerCount}`, () => {
    ...
    const startLabel = setupState.mode === "local"
        ? "Start Local"
        : setupState.role === "host"
            ? "Start Online Host"
            : "Start Online Join";
    ...
    if (onlineSession && setupState.mode === "online") {
        addButton(SETUP_CANCEL_ONLINE_X, SETUP_CANCEL_ONLINE_Y, SETUP_CANCEL_ONLINE_W, SETUP_CANCEL_ONLINE_H, "cancel-online", "Cancel Online Session", () => {
            resetOnlineSession();
...
function getMousePositionInCanvasSpace(mouse) {
...
export function onMouseDown(mouse) {
    ...
    for (const target of targets) {
        if (!inRange(x, y, target.x, target.y, target.w, target.h))
            continue;
        if (target.key)
            pressedKeys.add(target.key);
        target.onDown();
        break;
    }
}
export function onMouseUp(_mouse) {
    pressedKeys.clear();
}
